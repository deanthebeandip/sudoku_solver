	...
	def suspectlist(self):
		...
		for i in range(len(self.data)):
			...
			if len(real_sus) == 1 and self.data[i].value == 0: #if only one sus, then fill in val, remove suslist
				...
				#question: clear the sus list or else itll read...
			else:
				self.data[i].suslist  = real_sus
...
while(g.g_sum() < 404 and count < 20):
	count+=1
	g.row_unique()
	g.safesuspectlist() #fill in safe suspects...

	g.suspectlist()
	g.safesuspectlist() #fill in safe suspects again
	...
	g.suspectlist()
	g.safesuspectlist() #fill in safe suspects again
	print("Count is: ", count, ", " , int((g.g_sum() / 405) * 100 ), "%")
	mat_print(g.package())
